...
- The function returns `{ categories, source }` where `categories` are the exact spec names; a name→ID map in `src/lib/categories.ts` translates them to existing registry IDs (e.g. `"Risk & Uncertainty"` → `risk`). Unknown names are ignored.
